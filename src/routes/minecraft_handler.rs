...
pub async fn get_modrinth_mods(mod_ids: Vec<String>) -> Result<Vec<ModpackInfo>, AppError> {
...
pub async fn get_curseforge_mods(
    mod_ids: Vec<i64>,
    state: &AppState,
) -> Result<Vec<ModpackInfo>, AppError> {
    let mut mods = Vec::new();
    if !mod_ids.is_empty() {
        let client = reqwest::Client::new();
        let response = client
            .post("https://api.curseforge.com/v1/mods")
            .header("x-api-key", state.forge_api_key.as_str())
            .json(&json!({
                "modIds": mod_ids,
            }))
            .send()
...
pub async fn read_modpack(state: &AppState) -> Result<Vec<ModpackInfo>, AppError> {
    ...
    for file in mod_files {
        ...
        if let Some(update_section) = mod_file.get("update") {
            match (
                ...
                (_, _) => {
                    tracing::error!("Unexpected update section without curseforge or modrinth")
                }
            }
        } else {
            // Hosted elsewhere
    ...
    let mut curseforge_mods = get_curseforge_mods(cf_mods, state).await?;
...
pub async fn modpack_info_endpoint(
    State(state): State<AppState>,
) -> Result<Html<String>, AppError> {
    let mods = read_modpack(&state).await?;
