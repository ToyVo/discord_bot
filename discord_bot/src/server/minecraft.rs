use crate::{error::AppError, server::models::GameServer, state::AppState};

async fn track_generic<S: AsRef<str>>(
    _minecraft_address: S,
    _channel_id: S,
    server: GameServer,
    _state: &AppState,
) -> Result<(), AppError> {
...
pub async fn track_players() -> Result<(), AppError> {
    if let Some(state) = crate::state::GLOBAL_STATE.get() {
        ...
        Err(AppError::Other("Global state not initialized".to_string()))
    }
}
